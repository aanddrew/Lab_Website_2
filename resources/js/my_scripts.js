...
function loadStatsPage() {
    var buff_wins = 0;
    var buff_losses = 0;
    table = document.getElementById("stats_table").getElementsByTagName('td');
    for(var i = 0; i < table.length; i+= 5) {
        var our_score = table[i + 2].innerHTML;
        var their_score = table[i+3].innerHTML;
        console.log(our_score + " " + their_score);
        if (our_score > their_score) {
            table[i + 4].innerHTML = "Buffaloes";
            buff_wins++;
        }
        else {
            table[i + 4].innerHTML = table[i+1].innerHTML;
            buff_losses++;
        }
    }
